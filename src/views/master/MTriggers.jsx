import { Card, Banner, Timeline } from '../../components/Ui.jsx';
    ...
    <>
      ...
      <Card title="Dated reasons to make contact" hint={`${t.length} tracked`}>
        <Timeline>
          {t.map(([l, d, dt, k], i) => (
            <li key={i}>
              <span className="d num">{when(d)}</span>
              <span className="t">
                <span className={`dt ${k === 'money' ? 'g' : k === 'portfolio' ? 'o' : ''}`.trim()} />
                {l}
                <div className="sub2">{dt} · {k}</div>
              </span>
            </li>
          ))}
        </Timeline>
